...
COV_Cutoffs = {
    ...
    'carbon_24': {'struc': 0.8, 'comp': 4},
    'perov_5': {'struc': 0.2, 'comp': 5},
}
...
def main(args):
    ...
    if 'gen' in args.tasks:
        ...
        crys_array_list, _ = get_crystal_array_list(gen_file_path,0)
        ...
        elif 'recon' not in args.tasks:
            _, true_crystal_array_list = get_crystal_array_list(recon_file_path,0)
    ...
    if 'disp' in args.tasks:
        disp_file_path = get_file_paths(args.root_path, 'disp', args.label)
        dcrys_array_list, _ = get_crystal_array_list(disp_file_path, 0)
        data = load_data(disp_file_path)
        mat_ids = data["mat_ids"]
        sam_ids = data["sam_ids"]
        dgen_crys = p_map(lambda x: Crystal(x), dcrys_array_list)

        print("Save the generated Images...")
        for i in range(len(dgen_crys)) :
            crystal = dgen_crys[i]
            mat_id = mat_ids[i]
            sam_id = sam_ids[i].item()
            sample_path = str(args.root_path)+'sampled/'+str(mat_id)
            if not os.path.exists(sample_path):
                os.makedirs(sample_path)
            cfname = sample_path+"/"+str(sam_id)+".cif"
            pfname = sample_path+"/"+str(sam_id)+".png"
            crystal.structure.to(filename=cfname)
            cif2png(cfname, pfname)
